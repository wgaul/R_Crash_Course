...
## Introduction
The first step of data analysis in R is often to load data into R.  This document present four practice tasks to help you practice loading data into R.  Each practice task requires you to download a different dataset onto your computer and then load the data into R.  The third and fourth tasks are more difficult and require you to overcome some common challenges that arrise when loading data into R.
...
```
5) How many rows and columns are there in the data that you read into R? (use `str()` or `dim()`, you should find `r nrow(plant_df)` rows and `r ncol(plant_df)` columns).
6) What are the names of the columns?  Modify the following line of code to work to use the name of your data frame, and then paste the code into your R script and run it:
```{r, echo = T, eval=F}
colnames(plant_df)
```
