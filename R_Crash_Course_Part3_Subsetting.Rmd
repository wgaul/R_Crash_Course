...
This document starts by giving you each line of code that I ran during the demonstration at the start of the workshop.  You should write each of the lines of code yourself into your own script, run them, and make sure you understand what they do.  Then, starting on step **??** are new tasks but no code to complete them.  However, this document *does* show some output showing what it should look like if you do the task correctly.  You should write comments and code to do these tasks, and make sure your results match the results shown in this document.  
...
b) subset the males-only data to get only data for people who wear contacts.  This should give you a data frame with `r nrow(mc)` rows and `r ncol(mc)` columns.  The mean age of males who wear contacts should be `r mean(mc$AGE, na.rm = T)`, the mean height should be `r mean(mc$HEIGHT, na.rm = T)`, and the mean weight should be `r mean(mc$WEIGHT, na.rm = T)`.  (Hint: if you use the `mean()` function to calculate the mean, make sure to use the argument `na.rm = TRUE` to exclude missing (NA) values. So the code would look something like this: `mean(male_ht$HEIGHT, na.rm = TRUE)`).
...
- To subset data frames, use code similar to: `my_data[rows, columns]`
